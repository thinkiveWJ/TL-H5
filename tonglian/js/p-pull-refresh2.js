...
(function(){
	...
	var reset = function(isAnim) {
        ...
        options.$loadingEl.animate({'margin-left': -loadingW}, (isAnim == false ? 0 : 800), function() {
    		isTouching = false;
    		runCb('end');
    	});
		/**2017-06-02 start wj**/
		// options.$loadingEl.animate({'margin-left': "5px"}, (isAnim == false ? 0 : 800), function() {
		// 	isTouching = false;
		// 	runCb('end');
		// });
		/***2017-06-02 end wj****/
	...
	var touchStart = function(evt) {
		var scrollLeft = parseInt(options.$el.scrollLeft());
	...
	var touchMove = function(evt) {
		...
		var $loadingEl = options.$loadingEl,
        	...
        	t = x - startX;			// 触摸距离
        //  距离必须大于灵敏距离触摸才生效
        if(!isValid && t > options.startPX && $("section.highchart").scrollLeft() == 0 ) {
        	isValid = true;
			runCb('pullStart');
        }
        ...
        // 向上拖动
        if(t < 0){
        	$loadingEl.css('margin-left', -loadingW);
        }

		// 拖动高度未超过刷新显示容器高度
		if(t > 0 && t <= loadingW){
			$loadingEl.css('margin-left', -(loadingW-t));
			$loadingEl.width(loadingW);
		}
		// 拖动高度超过刷新显示容器高度
		else if(t > loadingW){
			$loadingEl.css('margin-left', 20);
			$loadingEl.width(t);
		}
        disY = t;
	}
	...
	var touchEnd = function(evt) {
	    ...
	    if(t <= loadingW){
	    	$loadingEl.css('margin-left', -(loadingW-t));
	    	$loadingEl.width(loadingW);
	    	$loadingEl.animate({'margin-left': -loadingW}, 200, function() {
	    		isTouching = false;
	    	});
	    }
	    ...
	    // 拖动高度超过刷新显示容器高度
	    	...
	    	$loadingEl.animate({'width': loadingW}, 200, function() {});
	    }
	    ...
	    if(options.url){
	    	runCb('start');
			if($("section.highchart").scrollLeft() != 0){
				return;
			}
	...
	var initlize = function() {
		loadingW = options.$loadingEl.width();
