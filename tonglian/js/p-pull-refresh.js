...
(function(){
	...
	var reset = function(isAnim) {
    	...
    	// });
		/**2017-06-02 start wj**/
		options.$loadingEl.animate({'margin-top': "5px"}, (isAnim == false ? 0 : 800), function() {
			isTouching = false;
			runCb('end');
		});
		/***2017-06-02 end wj****/
	...
	var touchMove = function(evt) {
        ...
        //  距离必须大于灵敏距离触摸才生效
        if(!isValid && t > options.startPX && options.$loadingEl.offset().top !== "5") {
        	isValid = true;
        	runCb('pullStart');
        }
	...
	var touchEnd = function(evt) {
	    ...
	    if(options.url){
	    	runCb('start');
			if(options.$loadingEl.offset().top === "5"){
				return;
			}
